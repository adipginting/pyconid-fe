...
import { getDetailUser } from "~/api/endpoint/.server/user";
...
import type { Route } from "./+types/dashboard-public";
...
export const loader = async ({ request, params }: Route.LoaderArgs) => {
	...
	try {
		...
		const dataUserProfile = await getDetailUser({ request, user_id: id });
		let userProfile = null;
		if (dataUserProfile.status === 401) {
			return redirect("/login");
		} else if (dataUserProfile.status === 404) {
			userProfile = null;
		} else if (dataUserProfile.status === 200) {
			const jsonUserProfile = await dataUserProfile.json();
			console.log(jsonUserProfile);
			userProfile = getUserProfileSchema.parse(jsonUserProfile);
		}
...
export default function DashboardPage(componentProps: Route.ComponentProps) {
	return (
		<main className="bg-[#FAF9F7]">
			<Header />
			{componentProps.loaderData.userProfile ? (
				<DashboardSection
